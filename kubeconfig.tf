resource "null_resource" "wait_cluster_ready" {
  provisioner "local-exec" {
    command = "until (aws s3 cp s3://${var.s3_bucket}/${var.cluster_name}/${local.s3_kubeconfig_filename} /tmp/kubeconfig_tmp && for i in $(seq 10); do kubectl version --kubeconfig /tmp/kubeconfig_tmp --request-timeout=5s || exit 1; sleep 1; done) >/dev/null 2>&1; do sleep 1; echo waiting for kubeconfig; done"
  }
  depends_on = [
